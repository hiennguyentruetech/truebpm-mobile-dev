...
class ELeaveDetailsTabBody extends CoreTabBody {
  ...
  @override
  CoreTabBodyState<ELeaveDetailsTabBody> createState() =>
      _ELeaveDetailsTabBodyState();
}

class _ELeaveDetailsTabBodyState
    extends CoreTabBodyState<ELeaveDetailsTabBody> {
  ...
  void _onChanged(String key, dynamic value) {
    setState(() {
      ...
      if (key == 'leaveType') {
        ...
        final isNew =
            (_moduleData['id'] == null ||
                _moduleData['id'].toString().isEmpty) &&
            (_moduleData['code'] == null ||
                _moduleData['code'].toString().isEmpty);
  ...
  void _updateLeaveBalanceFromItemDetail() {
    ...
    if (value != null && value is Map<String, dynamic>) {
      setState(() {
        _leaveBalance = {
          'TotalRemainLeave':
              value['totalRemainLeave'] ?? value['TotalRemainLeave'] ?? 0.0,
          'TotalLeaveApplied':
              value['totalLeaveApplied'] ?? value['TotalLeaveApplied'] ?? 0.0,
        };
      });
    } else {
      // Fallback to default values if no data available
      setState(() {
        _leaveBalance = {'TotalRemainLeave': 0.0, 'TotalLeaveApplied': 0.0};
      });
    }
  }
  ...
  Future<void> _loadLeaveTypeStats(String leaveTypeId) async {
    ...
    try {
      ...
      if (res1['success'] == true && res1['data'] != null) {
        ...
        if (data is List && data.isNotEmpty) {
          final first = data[0];
          nextUsedLeaveDays = first is Map && first['UsedDay'] != null
              ? num.tryParse(first['UsedDay'].toString())
              : null;
        } else if (data is Map &&
            data['data'] is List &&
      ...
      if (res2['success'] == true && res2['data'] != null) {
        ...
        if (data is List && data.isNotEmpty) {
          final first = data[0];
          nextTotalDayPerYear = first is Map && first['TotalDayPerYear'] != null
              ? num.tryParse(first['TotalDayPerYear'].toString())
              : null;
        } else if (data is Map &&
            data['data'] is List &&
            (data['data'] as List).isNotEmpty) {
  ...
  Widget _buildLeaveBalanceCards() {
    ...
    final totalRemain =
        _formatDecimal(_leaveBalance['TotalRemainLeave']) ??
        _formatDecimal(_moduleData['totalRemainLeave']) ??
        '0.00';
    final totalApplied =
        _formatDecimal(_leaveBalance['TotalLeaveApplied']) ??
        _formatDecimal(_moduleData['totalLeaveApplied']) ??
        '0.00';
  ...
  bool _shouldShowLeaveTypeInfoCards() {
    ...
    final String? ltId = (lt is Map && lt['id'] != null)
        ? lt['id'].toString()
        : null;
    final String ltName = (lt is Map && lt['name'] != null)
        ? lt['name'].toString()
        : '';
    final bool isAnnual =
        (ltId == 'FFFFD914-6057-4286-A321-773680D400A9') ||
        (ltName.toLowerCase() == 'annual');
  ...
  Widget _buildLeaveTypeInfoCards() {
    ...
    return Padding(
      ...
      child: Row(
        children: [
          Expanded(
            child: LeaveTypeStatCard(
              title: 'Used Days',
              value: used,
              icon: Icons.event_busy,
              isLoading: _isLoadingLeaveTypeStats,
              gradient: const LinearGradient(
    ...
    return CardSection(
      ...
      children: [
        ...
        ...CoreDynamicFields.buildFields(
          fieldConfigs: ELeaveDtlsFieldConfigs.generalInfoPrimaryFields(),
        ...
        ...CoreDynamicFields.buildFields(
          fieldConfigs: ELeaveDtlsFieldConfigs.generalInfoSecondaryFields(),
          itemDetail: _itemDetail,
          moduleData: _moduleData,
          onChanged: _onChanged,
        ),
  ...
  Widget _buildSystemInfoSection() {
    return CardSection(
      ...
      children: [
        ...CoreDynamicFields.buildFields(
          fieldConfigs: ELeaveDtlsFieldConfigs.systemInfoFields(),
          itemDetail: _itemDetail,
          moduleData: _moduleData,
          onChanged: _onChanged,
        ),
