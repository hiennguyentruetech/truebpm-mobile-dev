...
class CoreDynamicFields {
  ...
  static List<Widget> buildFields({
    ...
    final Map<String, dynamic> contextMap = {
      ...moduleData,
      ...(itemDetail['value'] is Map<String, dynamic> ? itemDetail['value'] as Map<String, dynamic> : <String, dynamic>{}),
    };
    ...
    for (Map<String, dynamic> original in fieldConfigs) {
      ...
      if (config.containsKey('visibleWhen')) {
        ...
        shouldInclude = moduleData.containsKey(fieldName) || isCommonField(fieldName);
      }

      if (!shouldInclude) continue;

      // Resolve dynamic templates in data endpoints (e.g., '.../API?x={{a.b}}')
      if (config['data'] is String && (config['data'] as String).contains('{{')) {
      ...
      final List<String> clearOnChange = List<String>.from(config['clearOnChange'] ?? const <String>[]);
      ...
      final String widgetType = config['widget'] ?? 'input'; // 'input', 'select', 'datetime', 'checkbox', 'collection', 'status'
      switch (widgetType) {
        case 'select':
          fields.add(_buildSelectField(config, itemDetail, wrappedOnChanged, getDefaultLabel));
          break;
        case 'datetime':
          fields.add(_buildDateTimeField(config, itemDetail, wrappedOnChanged, getDefaultLabel));
          break;
        case 'checkbox':
          fields.add(_buildCheckboxField(config, itemDetail, wrappedOnChanged, getDefaultLabel));
          break;
        case 'collection':
          fields.add(_buildCollectionField(config, itemDetail, wrappedOnChanged, getDefaultLabel));
          break;
        case 'tree':
          fields.add(_buildTreeField(config, itemDetail, wrappedOnChanged, getDefaultLabel));
          break;
        case 'status':
          fields.add(_buildStatusField(config, itemDetail, wrappedOnChanged, getDefaultLabel));
          break;
        default:
          fields.add(_buildInputField(config, itemDetail, wrappedOnChanged, getDefaultLabel));
      }
    }
  ...
  static Widget _buildSelectField(
    ...
    final String? label = config['label'];
    final String? hintText = config['hintText'];
    final String selectTypeStr = config['selectType'] ?? 'select'; // 'select' or 'dropdown'
  ...
  static Widget _buildDateTimeField(
    ...
    final String? label = config['label'];
    final String? hintText = config['hintText'];
    final String datetimeTypeStr = config['datetimeType'] ?? 'date'; // 'date', 'datetime', 'time', 'daterange'
  ...
  hidden: config['hidden'],
      onChanged: datetimeType != CoreDateTimeType.daterange 
          ...
          : null,
      onStartDateChanged: datetimeType == CoreDateTimeType.daterange && startDateKey != null
          ? (value) => onChanged(startDateKey, value)
          : null,
      onEndDateChanged: datetimeType == CoreDateTimeType.daterange && endDateKey != null
  ...
  static Widget _buildCheckboxField(
    ...
    final String checkboxStyleStr = config['checkboxStyle'] ?? 'material'; // 'material', 'custom', 'switch'
    final String positionStr = config['position'] ?? 'leading'; // 'leading', 'trailing'
  ...
  static Widget _buildCollectionField(
    ...
    final String? label = config['label'];
    final String? hintText = config['hintText'];
    final List<Map<String, dynamic>> children = List<Map<String, dynamic>>.from(config['children'] ?? []);
  ...
  static Widget _buildTreeField(
    ...
    final List<Map<String, dynamic>>? footerActions = (config['footerActions'] as List?)?.cast<Map<String, dynamic>>();
    final onFooterAction = config['onFooterAction'] as void Function(BuildContext, Map<String, dynamic>, Map<String, dynamic>)?;
    final List<String>? commonFields = (config['commonFields'] as List?)?.cast<String>();
    ...
    final Map<String, dynamic>? permissions = config['permissions'] as Map<String, dynamic>?;
    ...
    for (String fieldName in moduleData.keys) {
      if (requiredFields[fieldName] == true) {
        ...
        if (value == null || value.isEmpty) {
          final label = getCustomLabel?.call(fieldName) ?? getDefaultLabel(fieldName);
  ...
  static const List<String> defaultCommonFields = [
    'code', 'name', 'moduleCode', 'description', 'createdBy', 'createdDate'
  ];
    ...
    for (final cond in conditions) {
      ...
      switch (op) {
        ...
        case 'notExists':
          ...
          break;
        default:
          // Unknown operator -> do not hide by default
          break;
      }
    }
    return true;
